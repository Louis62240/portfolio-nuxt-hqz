    ...
    createSketch(p5) {
      ...
      const sketch = function(p) {
        ...
        function updateCanvasSize() {
          ...
          fArray = [];
          const numFocalPoints = 150; // Réduit pour accélérer
        ...
        p.draw = function() {
          if (hatchToggle) {
            // Augmenté à 10 itérations par frame pour accélérer le dessin
            for(let k = 0; k < 40; k++) {
              for (let j = 0; j < fArray.length; j++) {
                circleHatch(fArray[j].x, fArray[j].y);
              }
              if (hatchSize > 20) {
                hatchSize -= 20; // Réduction plus rapide
              }
            }
          }
        };
        ...
        function circleHatch(cx, cy) {
          ...
          chatchSize += 2; // Augmentation pour accélérer l'évolution
        }
